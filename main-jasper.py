...
# Load DATA
abundance_GT = torch.from_numpy(sio.loadmat('data/jasper/A_true.mat')['A_true']).permute(2, 0,
                                                                                         1)  # true abundance  4*100*100
original_HSI = torch.from_numpy(
    sio.loadmat('data/jasper/Y_clean.mat')['Y_clean']).permute(2, 0, 1)  # mixed abundance    198*100*100

band_Number = original_HSI.shape[0]  # 198
endmember_number, _, _ = abundance_GT.shape  # 14  100  100
EPS = 1e-6
col = 100
...
VCA_endmember, _, _ = hyperVca(masked_data.T, endmember_number)  # 156*3
...
# my net
class MG_Net_jasper(nn.Module):
    def __init__(self):
        super(MG_Net_jasper, self).__init__()
        ...
        self.Conv = nn.Sequential(
            ...
            nn.BatchNorm2d(128),
            nn.Tanh(),
            nn.Conv2d(in_channels=128, out_channels=band_Number, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(band_Number),
            nn.Tanh(),
        )
        self.vtrans = MultiHeadAttention(band_Number, band_Number, band_Number, 300, 30, 0.1)
        self.encoder = nn.Sequential(
            nn.Conv2d(in_channels=band_Number, out_channels=300, kernel_size=1, stride=1),
            nn.BatchNorm2d(300),
            nn.ReLU(),
        )
        self.EDLClassifier = EDLClassifier(self.encoder, 300, 100, endmember_number, 0.2)
...
def weights_init(m):
    ...
    nn.init.kaiming_normal_(net.linear_encoder_layer[7].weight.data)
...
Y_jasper = original_HSI.detach().cpu().squeeze().numpy()
Y_jasper = Y_jasper.transpose(1, 2, 0)
linear_part, nonlinear_part = wavelet_transform_separation(Y_jasper, 0.1)
original_HSI = original_HSI.unsqueeze(0).type(torch.cuda.FloatTensor)
...
for epoch in range(EPOCH):
    for i, x in enumerate(train_loader):
        ...
        MSELoss3_linear = rmse_loss(linear_recon, nonlinear_part, (1 - total_uncertainty))
        KLD_Loss = 0.1 * KLD_loss(evidence)
        loss_orth = 0.002 * orthogonal_loss(x1.squeeze(), E.squeeze().T)
        loss_CE = 10 * criterion(probabilities, x1)
        Overall_recon_loss = 0.5 * abundanceLoss3 + 5 * MSELoss3
        Linear_recon_loss = 0.5 * abundanceLoss3_linear + 5 * MSELoss3_linear
        Abundance_model_loss = 1 * (Linear_recon_loss + KLD_Loss + loss_CE)
        total_loss = Overall_recon_loss + loss_orth + Abundance_model_loss
        optimizer.zero_grad()
        total_loss.backward()
        optimizer.step()

        if (epoch + 1) % 100 == 0:
            print("******MSELoss3", MSELoss3.cpu().data.numpy(), "******abundanceLoss3",
                  abundanceLoss3.cpu().data.numpy())
...
# 保存模型权重
# torch.save(net.state_dict(),"./results/jasper/model_weights.pth")

# 加载模型权重
# net.load_state_dict(torch.load("./results/jasper/model_weights.pth"))
# net.eval()
...
sio.savemat('results/jasper/jasper.mat', {'A': en_abundance, 'E': decoder_para})
