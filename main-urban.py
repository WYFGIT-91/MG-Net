...
# Load DATA
abundance_GT = np.reshape(sio.loadmat('data/urban/end4_groundTruth.mat')['A'],
                          [4, 307, 307])  # true abundance  4*100*100
abundance_GT = torch.from_numpy(abundance_GT)
original_HSI = torch.from_numpy(
    sio.loadmat('data/urban/Y_clean.mat')['Y_clean']).permute(2, 0, 1)  # mixed abundance    162*100*100

band_Number = original_HSI.shape[0]  # 162
...
def plot_endmember(endmember_input, endmember_GT):
    ...
    for i in range(0, endmember_number):
        ...
        # plt.ylim(0, 0.6)  # 设置每个子图的 Y 轴范围为 0 到 1
    plt.show()
...
def conv55(inchannel, outchannel):
    return nn.Conv2d(inchannel, outchannel, kernel_size=5, stride=1, padding=2)
...
# my net
class MG_Net_urban(nn.Module):
    def __init__(self):
        super(MG_Net_urban, self).__init__()
        self.linear_encoder_layer = nn.Sequential(
            conv55(band_Number, 96),
            nn.BatchNorm2d(96),
            nn.LeakyReLU(0.2),
            nn.Dropout(drop_out),
            conv33(96, 48),
        ...
        self.Conv = nn.Sequential(
            ...
            nn.BatchNorm2d(128),
            nn.Tanh(),
            conv11(128, band_Number),
            nn.BatchNorm2d(band_Number),
            nn.Tanh(),
        )
        self.vtrans = MultiHeadAttention(band_Number, band_Number, band_Number, 300, 30, 0.1)
        self.encoder = nn.Sequential(
            nn.Conv2d(in_channels=band_Number, out_channels=256, kernel_size=1, stride=1),
            nn.BatchNorm2d(256),
            nn.LeakyReLU(),
        )
        self.EDLClassifier = EDLClassifier(self.encoder, 256, 64, endmember_number, 0.2)
...
Y_urban = original_HSI.detach().cpu().squeeze().numpy()
Y_urban = Y_urban.transpose(1, 2, 0)
linear_part, nonlinear_part = wavelet_transform_separation(Y_urban, 0.25)
...
for epoch in range(EPOCH):
    for i, x in enumerate(train_loader):
        ...
        loss_CE = 2 * criterion(probabilities, x1)

        Overall_recon_loss = 1 * abundanceLoss3 + 1 * MSELoss3
        Linear_recon_loss = (1 * abundanceLoss3_linear + 1 * MSELoss3_linear)
        ...
        if (epoch + 1) % 100 == 0:
            print("******MSELoss3", MSELoss3.item(), "******abundanceLoss3", abundanceLoss3.item(), )
...
# 保存模型权重
torch.save(net.state_dict(),"./results/urban/model_weights.pth")

# 加载模型权重
# net.load_state_dict(torch.load("./results/urban/model_weights.pth"))
# net.eval()
...
sio.savemat('results/urban/urban.mat', {'A': en_abundance, 'E': decoder_para})
