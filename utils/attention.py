...
def transpose_output(X, num_heads):
    ...
    # print('X2', X.shape)
    return X.reshape(X.shape[0], -1, X.shape[1], X.shape[1])
...
class MultiHeadAttention(nn.Module):
    def __init__(self, key_size, query_size, value_size, num_hiddens,
        ...
        self.W_o = nn.Conv2d(num_hiddens, query_size, 1, 1)
    ...
    def forward(self, queries, keys, values):
        ...
        # `num_hiddens` / `num_heads`)
        # print(queries.shape, self.query_size, self.num_heads, self.num_hiddens)
        # print(self.W_q(queries).shape)
